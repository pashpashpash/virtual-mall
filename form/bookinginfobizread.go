...
type BookingInfoBizReadForm struct {
	SourceAccount string `schema:"sourceaccount"`
	Message       string `schema:"message"`
	Signature     string `schema:"signature"`
}

func (me *BookingInfoBizReadForm) Validate() errorlist.Errors {
	errs := errorlist.New()

	validator.CheckNotEmpty(me.Message, "subject", errs)
	validator.CheckNotEmpty(me.Signature, "message", errs)
	validator.VerifyBizSignature(me.SourceAccount,
		me.Message, me.Signature, "signature", errs)

	return errs
}

func (me *BookingInfoBizReadForm) String() string {
	return "BookingInfo#" + me.SourceAccount
}

type BookingInfoAdminReadForm struct {
	SourceAccount string `schema:"sourceaccount"`
	Message       string `schema:"message"`
	Signature     string `schema:"signature"`
}

func (me *BookingInfoAdminReadForm) Validate() errorlist.Errors {
	errs := errorlist.New()
...
func (me *BookingInfoAdminReadForm) String() string {
	return "BookingInfo#" + me.SourceAccount
}

type BookingInfoAddForm struct {
	SourceAccount string `schema:"sourceaccount"`
	Message       string `schema:"message"`
	Signature     string `schema:"signature"`
}

func (me *BookingInfoAddForm) Validate() errorlist.Errors {
	errs := errorlist.New()
...
func (me *BookingInfoAddForm) ParseBookingInfo() (*protobuf.BookingInfo, error) {
	hexData := "0x" + strings.TrimPrefix(me.Message, "Booking Info: ")
...
func (me *BookingInfoAddForm) String() string {
